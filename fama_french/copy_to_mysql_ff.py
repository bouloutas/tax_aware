...
print("Script 'copy_to_mysql_ff.py' started.")
...
SNOWFLAKE_ACCOUNT = 'fpb76675.us-east-1' # Hardcoded account ID
# Password is not used for key-pair authentication

SNOWFLAKE_DATABASE = 'FAMA_FRENCH'
SNOWFLAKE_SCHEMA = 'PROCESSED_COMPUSTAT_DATA'
SNOWFLAKE_SOURCE_TABLE = 'DATA_FOR_FACTOR_CONSTRUCTION'

# --- MySQL Credentials (from .env or defaults) ---
...
MYSQL_TABLE_TARGET = 'data_for_factor_construction'
MYSQL_SOCKET = os.getenv('MYSQL_SOCKET_LOCAL', '/Users/tasosbouloutas/mysql_data/mysql.sock')

# --- Corrected Credential Check ---
print("--- Configuration Loaded ---")
print(f"Target Snowflake Table: {SNOWFLAKE_DATABASE}.{SNOWFLAKE_SCHEMA}.{SNOWFLAKE_SOURCE_TABLE}")
print(f"Target MySQL Table: {MYSQL_DATABASE_TARGET}.{MYSQL_TABLE_TARGET}")
...
print(f"MySQL Socket: {MYSQL_SOCKET}")
...
def create_mysql_database_if_not_exists(db_name):
    ...
    try:
        cnx_admin = mysql.connector.connect(
            user=MYSQL_USER, password=MYSQL_PASSWORD, host=MYSQL_HOST, unix_socket=MYSQL_SOCKET
        )
        cursor_admin = cnx_admin.cursor()
        cursor_admin.execute(f"CREATE DATABASE IF NOT EXISTS `{db_name}` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;")
        cursor_admin.close()
        cnx_admin.close()
...
def get_mysql_connection(db_name=MYSQL_DATABASE_TARGET):
    ...
    try:
        cnx = mysql.connector.connect(
            user=MYSQL_USER, password=MYSQL_PASSWORD, host=MYSQL_HOST, unix_socket=MYSQL_SOCKET, database=db_name
        )
...
def create_mysql_target_table_if_not_exists(cnx_mysql, table_name):
    cursor = cnx_mysql.cursor()
    create_table_query = f"""
    CREATE TABLE IF NOT EXISTS `{table_name}` (
        GVKEY VARCHAR(6), IID VARCHAR(3), RETURN_MONTH_END_DATE DATE,
        FORMATION_YEAR_T INT, MONTHLY_RETURN DECIMAL(20, 10), EXCHG VARCHAR(10),
        SIC VARCHAR(4), ME_JUNE DECIMAL(38, 4), BE_FY_T_MINUS_1 DECIMAL(38, 4),
        OP_FY_T_MINUS_1 DECIMAL(20, 10), INV_FY_T_MINUS_1 DECIMAL(20, 10),
        BM_T_MINUS_1 DECIMAL(20, 10),
        PRIMARY KEY (GVKEY, IID, RETURN_MONTH_END_DATE),
        INDEX idx_formation_year (FORMATION_YEAR_T),
        INDEX idx_gvkey_date (GVKEY, RETURN_MONTH_END_DATE)
    );
    """
    try:
        print(f"Ensuring MySQL table '`{table_name}`' exists...")
        cursor.execute(create_table_query); cnx_mysql.commit()
    except mysql.connector.Error as err:
...
def load_data_to_mysql(mysql_conn, df, table_name, chunk_size=10000):
    if df.empty:
        print("DataFrame is empty. No data to load to MySQL."); return

    cursor = mysql_conn.cursor()
    print(f"Clearing existing data from MySQL table '`{table_name}`' (using TRUNCATE)...")
    try:
...
def main():
    print("--- Starting Snowflake to MySQL Data Transfer for FF Base Data ---")
    ...
    try:
        create_mysql_target_table_if_not_exists(mysql_conn, MYSQL_TABLE_TARGET)
        query_sf = f"SELECT * FROM {SNOWFLAKE_SOURCE_TABLE};"
        ...
        if not df_from_snowflake.empty:
            ...
            if 'RETURN_MONTH_END_DATE' in df_from_snowflake.columns:
                 df_from_snowflake['RETURN_MONTH_END_DATE'] = pd.to_datetime(df_from_snowflake['RETURN_MONTH_END_DATE']).dt.date
            load_data_to_mysql(mysql_conn, df_from_snowflake, MYSQL_TABLE_TARGET)
        else:
            print("No data fetched from Snowflake or DataFrame is empty.")
    except Exception as e:
        import traceback
        print(f"An error occurred in main execution block: {e}"); traceback.print_exc()
    finally:
...
if __name__ == "__main__":
    main()
    print("--- Script 'copy_to_mysql_ff.py' Finished ---")